...
def run_experiment(COLUMN_SIZE,QUERY_PATTERN,QUERY_SELECTIVITY,ALGORITHM,NUM_QUERIES,FIXED_DELTA=None,INTERACTIVITY_THRESHOLD=None):
    COLUMN_PATH = column_path(COLUMN_SIZE)
...
def run_experiment_baseline(COLUMN_SIZE,QUERY_PATTERN,QUERY_SELECTIVITY,ALGORITHM,NUM_QUERIES):
    ...
    print(codestr)
    result = os.popen(codestr).read()
    cursor.execute('''INSERT INTO experiment_baseline(algorithm_id, workload_id, column_size, query_selectivity)
    ...
    experiment_id =  cursor.execute('''SELECT id FROM experiment_baseline where algorithm_id = (?) and workload_id=(?) and column_size=(?) and query_selectivity=(?)
    ...
    for query_number in range(0, len(result)-1):
        ...
        cursor.execute('''INSERT INTO times_baseline(experiment_id, query_number, time_seconds)
              VALUES(:experiment_id,:query_number, :time_seconds)''',
              {'experiment_id':experiment_id, 'query_number':query_number,'time_seconds':query_result[1]})
...
def run_experiment_progressive(COLUMN_SIZE,QUERY_PATTERN,QUERY_SELECTIVITY,ALGORITHM,NUM_QUERIES,FIXED_DELTA):
    ...
    print(codestr)
    result = os.popen(codestr).read()
    cursor.execute('''INSERT INTO experiment_progressive(algorithm_id, workload_id, column_size, query_selectivity,fixed_delta)
    ...
    experiment_id =  cursor.execute('''SELECT id FROM experiment_progressive where algorithm_id = (?) and workload_id=(?) and column_size=(?) and query_selectivity=(?) and fixed_delta=(?)
    ...
    for query_number in range(0, len(result)-1):
        query_result = result[query_number].split(";")
        cursor.execute('''INSERT INTO times_progressive(experiment_id, query_number, time_seconds)
              VALUES(:experiment_id,:query_number, :time_seconds)''',
              {'experiment_id':experiment_id, 'query_number':query_number, 'time_seconds':query_result[1]})
...
def run_experiment_cost_model(COLUMN_SIZE,QUERY_PATTERN,QUERY_SELECTIVITY,ALGORITHM,NUM_QUERIES,FIXED_INTERACTIVITY_THRESHOLD):
    ...
    print(codestr)
    result = os.popen(codestr).read()
    cursor.execute('''INSERT INTO experiment_progressive_cm(algorithm_id, workload_id, column_size, query_selectivity,fixed_interactivity_threshold)
    ...
    experiment_id =  cursor.execute('''SELECT id FROM experiment_progressive_cm where algorithm_id = (?) and workload_id=(?) and column_size=(?) and query_selectivity=(?) and fixed_interactivity_threshold=(?)
    ...
    for query_number in range(0, len(result)-1):
        query_result = result[query_number].split(";")
        cursor.execute('''INSERT INTO times_progressive_cm(experiment_id, query_number, delta, time_seconds)
              VALUES(:experiment_id,:query_number, :delta, :time_seconds)''',
              {'experiment_id':experiment_id, 'query_number':query_number, 'delta':query_result[0], 'time_seconds':query_result[1]})
        ...
        for query in WORKLOAD_LIST:
            for selectivity in QUERY_SELECTIVITY_LIST:
                ...
                for algorithm in ALGORITHM_LIST:
                    cursor.execute('''
                       SELECT id FROM experiment_baseline where algorithm_id = (?) and workload_id=(?) and column_size=(?) and query_selectivity=(?)
...
def template_run_progressive(ALGORITHM_LIST,DELTA_LIST=0,COLUMN_SIZE_LIST=0,WORKLOAD_LIST=0,QUERY_SELECTIVITY_LIST=0,INTERACTIVITY_THRESHOLD=0,NUM_QUERIES=10000):
    compile()
    if COLUMN_SIZE_LIST == 0:
        COLUMN_SIZE_LIST = [10000000,100000000,1000000000]
    if WORKLOAD_LIST == 0:
            ...
            for selectivity in QUERY_SELECTIVITY_LIST:
                ...
                for algorithm in ALGORITHM_LIST:
                    for delta in DELTA_LIST:
                        cursor.execute('''
                           SELECT id FROM experiment_progressive where algorithm_id = (?) and workload_id=(?) and column_size=(?) and query_selectivity=(?) and fixed_delta=(?)
...
def template_run_progressive_cost_model(ALGORITHM_LIST,DELTA_LIST=0,COLUMN_SIZE_LIST=0,WORKLOAD_LIST=0,QUERY_SELECTIVITY_LIST=0,INTERACTIVITY_THRESHOLD_LIST=0,NUM_QUERIES=10000):
    compile()
    if COLUMN_SIZE_LIST == 0:
        COLUMN_SIZE_LIST = [10000000,100000000,1000000000]
    if WORKLOAD_LIST == 0:
            ...
            for selectivity in QUERY_SELECTIVITY_LIST:
                ...
                for algorithm in ALGORITHM_LIST:
                    for interactivity_threshold in INTERACTIVITY_THRESHOLD_LIST:
                        cursor.execute('''
                           SELECT id FROM experiment_progressive_cm where algorithm_id = (?) and workload_id=(?) and column_size=(?) and query_selectivity=(?) and fixed_interactivity_threshold=(?)
...
def run_baseline():
    ...
    # NUM_QUERIES=0
    template_run_baseline(ALGORITHM_LIST)

def run_progressive():
    ALGORITHM_LIST = [ProgressiveQuicksort,ProgressiveRadixsortMSD,ProgressiveRadixsortLSD,ProgressiveBucketsortEquiheight]
    # COLUMN_SIZE_LIST=[]
    # WORKLOAD_LIST=[]
    # DELTA_LIST=[]
    # QUERY_SELECTIVITY_LIST=[]
...
def template_run_progressive_cost_model():
    ...
    # INTERACTIVITY_THRESHOLD_LIST=[]
    # NUM_QUERIES=0
    template_run_progressive_cost_model(ALGORITHM_LIST)
