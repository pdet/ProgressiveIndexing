...
# dev.off()
n <- 160000
skyserver_p <- head(read.csv("skyserver_queries_point.csv", header=F, sep=";", col.names=c("query", "start", "stop")), n)
skyserver_1 <- head(read.csv("skyserver_queries_1.csv", header=F, sep=";", col.names=c("query", "start", "stop")), n)
pdf("skyserver_p.pdf", width=8, height=5)
ggplot(skyserver_p, aes(query)) + geom_segment(aes(y=start, yend=stop, x=query,xend=query), size=1.2) + theme + scale_y_continuous(breaks=c(0, 359999999), labels=c("0", expression(359999999))) + xlab("Query (#)") + ylab("Query Range")
dev.off()

pdf("skyserver_1.pdf", width=8, height=5)
ggplot(skyserver_1, aes(query)) + geom_segment(aes(y=start, yend=stop, x=query,xend=query), size=1.2) + theme + scale_y_continuous(breaks=c(0, 359999999), labels=c("0", expression(10^8))) + xlab("Query (#)") + ylab("Query Range")
dev.off()