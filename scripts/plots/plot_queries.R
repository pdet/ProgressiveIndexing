...
QUERY_SELECTIVITY_PLOT <- .0001

pstchname = "P. Stc 10%[10]"

ORDERING = c("pstc"=1,"ppbeh"=2, 'ppcrk'=3, 'pprs'=4, 'ppbew'=5, 'stc'=6, 'std'=7, 'cgi'=8, 'fs'=9, 'fi'=10)
cols <- c("P. Stc 10%[10]"='#bdd09f',"P. Cracking"='#493829',"Cracking"='#8f3b1b',"P. Bucket"='#4e6172',"Stochastic"='#d57500',"Coarse"='#404f24',"P. Radix (LSD)"='#dbca69',"Index"='#b99c6b', "P. Radix (MSD)"='#855723', "Scan"='#a9a18c')

aggr %>% filter(query==1) -> pdata

pdata %>% mutate(label=recode(algorithm, ppbeh="P. Bucket", ppbew="P. Radix (MSD)", ppcrk="P. Cracking", pprs="P. Radix (LSD)", cgi="Coarse", std="Cracking", fs="Scan", stc="Stochastic", fi="Index", pstc=pstchname)) -> pdata2
...
aggr %>% filter(query_selectivity!=QUERY_SELECTIVITY_PLOT) -> pdata

pdata %>% mutate(label=recode(algorithm, ppbeh="P. Bucket", ppbew="P. Radix (MSD)", ppcrk="P. Cracking", pprs="P. Radix (LSD)", cgi="Coarse", std="Cracking", fs="Scan", stc="Stochastic", fi="Index", pstc=pstchname)) -> pdata2
pdata2 <- filter(pdata2, query < 1000)

fitimes <- pdata2 %>% filter(query > 1) %>% group_by(algorithm) %>% summarise(m=median(query_time))
...
pdata3 <- filter(pdata2, query < 10001) %>% arrange(ORDERING[algorithm]) 

pdata4 <- filter (pdata3, query == 9999)
