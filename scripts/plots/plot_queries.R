...
dd <- fread("output.csv")
dd %>%  group_by(algorithm, query_pattern) %>%  mutate(query = 1:n(), cumtime=cumsum(query_time)) %>% as.data.frame() -> ddq

ddq %>% select(algorithm, query, query_pattern, query_time, cumtime) %>% group_by(algorithm, query, query_pattern) -> aggr
...
pdata3 <- filter(pdata2, !(algorithm %in% c("p"))) %>% arrange(ORDERING[algorithm]) 
...
pdata %>% mutate(label=recode(algorithm, pqs="P. Quick", pqscm="P. Quick [Cost Model]", cgi="Coarse", std="Cracking", fs="Scan", stc="Stochastic", fi="Index", pstc="P. Stc 10%[10]")) -> pdata2
pdata2 <- filter(pdata2, query < 300)
...
pdata3 <- filter(pdata2, query < 160000) %>% arrange(ORDERING[algorithm]) 

pdata4 <- filter (pdata3, query == 160000)
