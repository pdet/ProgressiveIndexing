...
''')
cursor.execute('''
    CREATE TABLE column_distribution(id INTEGER PRIMARY KEY, name TEXT NOT NULL)
''')
cursor.execute('''
    CREATE TABLE experiments(id INTEGER PRIMARY KEY, algorithm_id INTEGER NOT NULL, workload_id INTEGER NOT NULL, INTEGER column_distribution_id NOT NULL, column_size INTEGER NOT NULL, query_selectivity REAL NOT NULL, fixed_delta REAL, fixed_interactivity_threshold REAL
    , FOREIGN KEY (workload_id) REFERENCES workload(id), FOREIGN KEY (algorithm_id) REFERENCES algorithm(id), FOREIGN KEY (column_distribution_id) REFERENCES column_distribution(id))
''')
cursor.execute('''
...
cursor.executemany(''' INSERT INTO column_distribution(name) VALUES(?)''', c_dist)
