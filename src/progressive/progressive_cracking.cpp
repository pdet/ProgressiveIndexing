...
double get_max_cracking_delta(Column &c, int64_t low, int64_t high, double delta) {
    bool initial_run = c.qs_index.root.left < 0;
    QuicksortNode& node = c.qs_index.root;
    double page_count = (c.data.size() / ELEMENTS_PER_PAGE);
    ...
    if (initial_run) {
        ...
        double pivot_speed = 2 * (READ_ONE_PAGE_SEQ_MS + WRITE_ONE_PAGE_SEQ_MS) * page_count;
        max_delta = ((1 - rho + alpha) * scan_speed - INTERACTIVITY_THRESHOLD) / (scan_speed - pivot_speed);
        max_delta = std::min(1 - alpha, max_delta);
    } else {
        ...
        double alpha = (double) (right_node.end - left_node.start) / (double) c.data.size();
        fprintf(stderr, "alpha %lf\n", alpha);

        max_delta = (lookup_speed + alpha * scan_speed - INTERACTIVITY_THRESHOLD) / (scan_speed - refine_speed);
    }
    fprintf(stderr, "Max proposed delta: %lf\n", max_delta);
    return max_delta;
}
