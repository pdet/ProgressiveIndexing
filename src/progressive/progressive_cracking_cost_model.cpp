...
void range_query_incremental_cracking_recursive_cost_model(Column& c, QuicksortNode& node, ResultStruct& results, int64_t low, int64_t high, ssize_t &remaining_swaps)
{
    ...
    if (node.left < 0) {
        ...
        if (low < pivot) {
            range_query_incremental_cracking_recursive_cost_model(
...
ResultStruct range_query_incremental_cracking_cost_model(Column& c, int64_t low, int64_t high, double delta)
{
    if (!c.qs_index.index) {
    ...
    if (initial_run) {
        ...
        range_query_incremental_cracking_recursive_cost_model(c, c.qs_index.root, results, low, high, remaining_swaps);
    }
...
QuicksortNode& FindLowestNode_cost_model(Column &c, QuicksortNode& node, int64_t value) {
    if (node.left < 0) {
        return node;
    }
    return FindLowestNode_cost_model(c, c.qs_index.nodes[value < node.pivot ? node.left : node.right], value);
}

size_t GetHeight_cost_model(Column &c, QuicksortNode& node) {
    if (node.left < 0) {
        return 1;
    }
    return 1 + std::max(GetHeight_cost_model(c, c.qs_index.nodes[node.left]), GetHeight_cost_model(c, c.qs_index.nodes[node.right]));
}
...
double get_estimated_time_cracking(Column &c, int64_t low, int64_t high, double delta) {
    ...
    if (initial_run) {
        ...
        size_t height = GetHeight_cost_model(c, node);
        double lookup_speed = height * RANDOM_ACCESS_PAGE_MS;
        double refine_speed = WRITE_ONE_PAGE_SEQ_MS * page_count;

        // figure out alpha
        auto left_node = FindLowestNode_cost_model(c, node, low);
        auto right_node = FindLowestNode_cost_model(c, node, high);
