...
//! f linear
offset_t FLINEAR(const working_area_t p, const offset_t numBits, const offset_t bMin, const offset_t bMax,
...
bool isPartitionSmallerEqualsThan(const working_area_t &p, const offset_t numElements) {
	...
	const bool isSmallerEquals =
	    (p.second.offsetWithLength.offset - p.first.offsetWithLength.offset + 1) <= numElements;
...
void inPlaceRadixPartitioning(crow_t *const c, const offset_t n, const offset_t numPartitions, offset_t *histogram,
	...
	if (shift == sizeof(entry_t) * 8) {
		...
		for (i = 0; i < n; ++i)
			++cnt[((c[i].col << shiftOffset) >> shift) & mask];
	}
...
void RADIX_helper(crow_t *dst, offset_t offset_L, offset_t offset_H, offset_t shiftOffset, crackerIndex_pt index,
                  offset_t numBits) {
...
working_area_t decision_tree_simple(crackerIndex_pt index, entry_t key, crow_t *dst, offset_t size, ADAPT_FUNC F,
	...
	} else if (isPartitionSmallerEqualsThan(p, sComplete) ||
	           (sizeof(entry_t) * 8) - p.first.offsetWithLength.entryLength <= bMin) {
...
entry_t DECISION_helper_simple(crow_t *dst, offset_t size, entry_t keyL, entry_t keyH, crackerIndex_pt index,
                               ADAPT_FUNC F, wdPartitioned_t *const workingData) {
	...
	if (isSorted(pLow)) {
		oFL = oL =
		    bin_search(keyL, dst, pLow.first.offsetWithLength.offset + 1, pLow.second.offsetWithLength.offset) - 1;
	...
	if (isSorted(pHigh)) {
		oFH = oH =
		    bin_search(keyH, dst, pHigh.first.offsetWithLength.offset + 1, pHigh.second.offsetWithLength.offset) - 1;
...
void inPlaceRadixPartitioningNoHist(crow_t *const c, const offset_t n, const offset_t numPartitions,
                                    offset_t *histogram, offset_t shiftOffset, offset_t shift) {
...
void EQUI_DEPTH_RADIX_helper(const crow_t *input, crow_t *output, offset_t size, offset_t *histogram, offset_t *keys,
	...
	for (offset_t j = 0; j < numPartitions; j++) {
		...
		if (keys[j] == NO_SPLIT_MARK)
			continue;

		//! else do in-place radix partitioning
		offL = (j > 0 ? histogram[j - 1] : 0);
		offH = histogram[j] - 1;
		inPlaceRadixPartitioningNoHist(output + offL, offH - offL + 1, numMinPartitions, equi_counters[i],
		                               (offset_t)(shiftOffset + numBits), shift2);
...
void GIVEN_HIST_OOP_RADIX_helper(crow_t *in, crow_t *out, offset_t offset_L, offset_t offset_H, offset_t shiftOffset,
                                 crackerIndex_pt index, offset_t numBits, offset_t *histogram) {
...
void EQUIDEPTHOOPRADIX_helper(crow_t *in, crow_t *out, offset_t size, offset_t shiftOffset, crackerIndex_pt index,
