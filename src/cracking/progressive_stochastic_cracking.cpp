...
IntPair mdd1rfull(IndexEntry*& c, int64_t posL, int64_t posH,  int64_t low, int64_t high, IndexEntry*& view, int64_t& view_size){

    int64_t L = posL; int64_t R = posH;
    int64_t a = low; int64_t b = high;
...
IntPair mdd1rp_split_and_materialize(IndexEntry*& c,pair<int64_t,pair<int64_t,int64_t> > &P, int64_t L, int64_t R, int64_t a, int64_t b, int64_t nswap,IndexEntry*& view, int64_t& view_size){
    int64_t X = P.first;
...
IntPair mdd1rp_find(IndexEntry *&c,int64_t L, int64_t R, int64_t a, int64_t b, int nswap,IndexEntry*& view, int64_t& view_size){
    IntPair pivot_pair;
...
AvlTree progressiveStochasticCracking(IndexEntry *&c, int64_t dataSize, AvlTree T, int64_t lowKey, int64_t highKey, QueryOutput *qo){
    IntPair p1,p2;
    ...
    if(p1->first==p2->first && p1->second==p2->second){
        pivot_pair = mdd1rp_find(c,p1->first, p1->second, lowKey, highKey,COLUMN_SIZE*ALLOWED_SWAPS_PERCENTAGE,qo->view1, qo->view_size1); // a = L b = R N = Column_size p = swap_percentage
        ...
        pivot_pair->first = NULL;
        pivot_pair->second = NULL;
        IntPair pivot_pair1 = mdd1rp_find(c,p1->first, p1->second, lowKey, highKey,COLUMN_SIZE*ALLOWED_SWAPS_PERCENTAGE,qo->view1, qo->view_size1);
        qo->middlePart = &c[p1->second+1];
        int size2 = p2->first-p1->second-1;
        qo->middlePart_size = size2;
        IntPair pivot_pair2 = mdd1rp_find(c,p2->first, p2->second, lowKey, highKey,COLUMN_SIZE*ALLOWED_SWAPS_PERCENTAGE,qo->view2, qo->view_size2);
