...
int64_t NUM_QUERIES;
int ALGORITHM, RUN_CORRECTNESS, INTERACTIVITY_IS_PERCENTAGE, RADIXSORT_TOTAL_BYTES;
...
void adaptive_adaptive_indexing(wdPartitioned_t *const workingData, const offset_t size,
                                const entry_pair_t *const queries, const offset_t numQueries,
	...
	for (current_query = 0; current_query < numQueries; current_query++) {
		...
		if (current_query == 0) {
			...
			start = chrono::system_clock::now();
			res = DECISION_helper_simple(dst, size, queries[current_query].first, queries[current_query].second + 1,
			                             index, F, workingData);
			//! process queries
			end = chrono::system_clock::now();
			query_times.idx_time[current_query].index_creation += chrono::duration<double>(end - start).count();
...
int main(int argc, char **argv) {
	...
	for (size_t i = 0; i < repetition; i++) {
		...
		case 13:
			...
			break;
		case 14:
			progressive_indexing_cost_model(c, rangequeries, answers, deltas, range_query_incremental_radixsort_msd,
