...
void do_hybrid_radixsort_insert(IndexEntry *c, unsigned int n, int shift) {
...
static void do_hybrid_radixsort_insert(int64_t *val, size_t *ind, unsigned int n, int shift) {
    ...
    memset(cnt, 0, 256*sizeof(unsigned int)); // Zero counters
    switch (shift) { // Count occurrences
        case 0: 	for(i=0;i<n;i++) cnt[val[i] & 0xFF]++; break;
        case 8: 	for(i=0;i<n;i++) cnt[(val[i] >> 8) & 0xFF]++; break;
        case 16: 	for(i=0;i<n;i++) cnt[(val[i] >> 16) & 0xFF]++; break;
        case 24: 	for(i=0;i<n;i++) cnt[val[i] >> 24]++; break;
            /*
             * 	Note that even though our radix sort implementations work on 8 byte keys, only 4 bytes are actually processed,
             * 	as our key domain is never lager than [0, 2^32-1]. In a system implementation, this would be handled before sorting by inspecting
             * 	the max value of the column in the statistics.
...
//
//// hybrid radix sort: switches to insertion sort after a threshold
//static void do_hybrid_radixsort_insert_copy(int64_t *data, size_t *ind, unsigned int n, const int64_t* val, size_t offset, int shift) {
//    unsigned int last[256],ptr[256],cnt[256];
//    unsigned int i,j,k,sorted,remain;
//    int64_t temp, swap;
//    size_t temp_i, swap_i;
...
//                    remain--;
//                }while(i != k);
//                data[j] = swap;	// Place last element in cycle
//                ind[j] = swap_i;
//            } else {
//                data[j] = val[j];
//                ind[j] = offset + j;
...
//        for(i=0; i<256; i++){
//            if (cnt[i] > INSERT_SORT_LEVEL)
//                do_hybrid_radixsort_insert(&data[last[i]-cnt[i]], &ind[last[i]-cnt[i]], cnt[i], shift);
//            else if(cnt[i] > 1)
//                insertion_sort(&data[last[i]-cnt[i]], &ind[last[i]-cnt[i]], cnt[i]);
//        }
//    }
//}
//
...
//    do_hybrid_radixsort_insert_copy(val, ind, n, &data[offset], offset, 24);
//    return true;
//}
