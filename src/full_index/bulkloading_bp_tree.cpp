...
void BPNode::removePointer(const int64_t position) {
	if (m_currentNumberOfNodes > 0) {
		memmove(m_pointers + position, m_pointers + position + 1,
		        sizeof(BPNode *) * (m_currentNumberOfNodes - position - 1));
	...
	for (int64_t i = 0; i < size;) {
		if (m_currentLeaf->isFull()) {
			...
			if (m_currentLeaf->m_fatherNode->isFull()) {
				//! there is no space left, so we have to split the father node recursively to be able to
				//! link the new leaf
				m_currentLeaf->m_fatherNode = m_currentLeaf->m_fatherNode->split(m_root);
			}

			//! there is space left, so we can simply add the new leaf there, but only if it is not an
			//! overflow leaf
...
BulkBPTree::BulkBPTree(int64_t *data, int64_t size) {
	...
	for (int i = 0; i < size;) {
		...
		//! space left in leaf, so simply insert. need to check on node size, i should jump from node
		//! size!!
		if (!overflow) {
