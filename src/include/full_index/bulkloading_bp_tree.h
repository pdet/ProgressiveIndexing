...
typedef enum _NodeType { InnerNodeType, LeafNodeType } NodeType;
...
protected:
	...
	BPNode(NodeType nodeType)
	    : m_nodeType(nodeType), m_currentNumberOfEntries(0), m_keys(NULL), m_currentNumberOfNodes(0), m_pointers(NULL),
	      m_fatherNode(NULL) {
	}
...
public:
	...
	void setAsOverflowNode() {
		m_isOverflowNode = true;
	}
};
